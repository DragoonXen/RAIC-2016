 ...
 */
public class YYY_Drawing_DrawPanel extends JPanel {
	...
	private List<YYY_Drawing_Figure> drawFigures;

	public YYY_Drawing_DrawPanel(double fullSizeX, double fullSizeY) {
		...
		drawFigures = new LinkedList<YYY_Drawing_Figure>();
	...
	private void recalcMultCoord() {
		...
		double multX = size.getWidth() / (YYY_Drawing_DrawPanel.this.drawWidth + 1);
		double multY = size.getHeight() / (YYY_Drawing_DrawPanel.this.drawHeight + 1);

		multCoord = Math.min(multX, multY);
		YYY_Drawing_DrawPanel.this.repaint();
	}

	private void zoomAction(int x, int y, boolean zoomIn) {
		double cX = x / multCoord;
		double cY = y / multCoord;
		cX += YYY_Drawing_DrawPanel.this.drawStartX;
		cY += YYY_Drawing_DrawPanel.this.drawStartY;
		if (zoomIn) {
	...
	public void paint(Graphics g) {
		...
		synchronized (this) {
			...
			for (YYY_Drawing_Figure drawFigure : drawFigures) {
	...
	public void addFigure(YYY_Drawing_Figure figure) {
		synchronized (this) {
