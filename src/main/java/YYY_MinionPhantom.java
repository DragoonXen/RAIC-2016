 ...
 */
public class YYY_MinionPhantom extends Minion {
	...
	public YYY_MinionPhantom(Minion minion, int tick, int line) {
		super(minion.getId(),
			  minion.getX(),
			  minion.getY(),
			  minion.getSpeedX(),
		...
		this.position = new YYY_Point(minion.getX(), minion.getY());
	...
	public YYY_MinionPhantom clone() {
		YYY_MinionPhantom mp = new YYY_MinionPhantom(this, lastSeenTick, line);
		mp.position = new YYY_Point(position.getX(), position.getY());
		return mp;
	}
}
