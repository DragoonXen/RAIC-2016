 ...
 */
public class YYY_Drawing_DrawingStrategy extends YYY_StrategyImplement {

	private List<YYY_Drawing_DrawingData> drawingDataList;
	private YYY_Drawing_MainFrame mainFrame;
	...
	private static YYY_Drawing_DrawingStrategy instance;
	protected TreeMap<Double, YYY_ScanMatrixItem> foundScanMatrixItems = new TreeMap<>();
	...
	private TreeMap<Integer, YYY_Pair<YYY_Point, Double>> fireballShots = new TreeMap<>();

	public YYY_Drawing_DrawingStrategy(Wizard self) {
		super(self);
		drawingDataList = Collections.synchronizedList(new ArrayList<>());
		YYY_Drawing_DrawingStrategy.instance = this;
	}

	public void move(int tick) {
		synchronized (this) {
			YYY_Drawing_DrawingData drawingData = drawingDataList.get(tick);
			if (drawingData == null) {
				mainFrame.getDrawPanel().clear();
				mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(0,
																		0,
																		YYY_Constants.getGame().getMapSize(),
																		YYY_Constants.getGame().getMapSize(),
																		Color.RED));
				mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(0,
																		YYY_Constants.getGame().getMapSize(),
			...
			this.lastTick = tick;
			YYY_Drawing_DrawingData currentData = applyData(drawingData, true);
	...
	private YYY_Drawing_DrawingData applyData(YYY_Drawing_DrawingData dataToApply, boolean receiveCurrent) {
		YYY_Drawing_DrawingData storedData = null;
		if (receiveCurrent) {
			storedData = getCurrentData(world, self);
		}
		YYY_Drawing_DrawingData currentDrawingData = dataToApply.clone();
		...
		YYY_PositionMoveLine.INSTANCE.updatePointToMove(currentDrawingData.getMoveToLinePoint());
		for (int i = 0; i != YYY_Constants.getLines().length; ++i) {
			YYY_Constants.getLines()[i].fightPoint.update(currentDrawingData.getLinesFightPoints()[i]);
		}
	...
	public YYY_Drawing_DrawingData getCurrentData(World world, Wizard self) {
		return new YYY_Drawing_DrawingData(self,
										   ...
										   lastFightLine,
										   myLineCalc,
										   YYY_PositionMoveLine.INSTANCE.getPositionToMove().clonePoint(),
										   new YYY_Point[]{YYY_Constants.getLines()[0].fightPoint, YYY_Constants.getLines()[1].fightPoint, YYY_Constants.getLines()[2].fightPoint},
	...
	public void move(Wizard self, World world, Game game, Move move) {
		synchronized (this) {
			if (mainFrame == null) {
				mainFrame = new YYY_Drawing_MainFrame(world.getWidth(), world.getHeight());
	...
	private void drawUnit(CircularUnit unit, YYY_Point position, Color color) {
		drawUnit(unit, position, null, color);
	}

	private void drawUnit(CircularUnit unit, YYY_Point position, Double visibleDistance, Color color) {
		if (position == null) {
			position = new YYY_Point(unit.getX(), unit.getY());
		}
		if (color == null) {
			color = YYY_Constants.getCurrentFaction() == unit.getFaction() ?
					Color.green :
					YYY_Constants.getEnemyFaction() == unit.getFaction() ?
							Color.red :
							Color.blue;
		}
		drawPanel.addFigure(new YYY_Drawing_Circle(position.getX(),
												   position.getY(),
												   unit.getRadius(),
												   true,
												   color));
		...
		if (unit instanceof Minion && ((Minion) unit).getType() == MinionType.FETISH_BLOWDART) {
			...
			angle += Math.PI / 2.;
			drawPanel.addFigure(new YYY_Drawing_Line(position.getX() + Math.cos(angle) * unit.getRadius(),
		...
		drawPanel.addFigure(new YYY_Drawing_Line(position.getX(),
												 position.getY(),
		...
		if (unit instanceof LivingUnit) {
			...
			drawPanel.addFigure(new YYY_Drawing_Polygon(polygonX, polygonY, true, Color.black));
			...
			drawPanel.addFigure(new YYY_Drawing_Polygon(polygonX, polygonY, true, Color.RED));
		}
		if (unit instanceof Wizard) {
			...
			polygonX = Arrays.copyOf(polygonX, polygonX.length);
			polygonX[2] = polygonX[3] = polygonX[0] + (livingUnit.getLife() / (double) livingUnit.getMaxLife()) * unit.getRadius() * 2.;
			drawPanel.addFigure(new YYY_Drawing_Polygon(polygonX, polygonY, true, Color.BLUE));
		}
	...
	private void move(Wizard self, World world, Game game, Move move, boolean draw) {
		...
		if (draw) {
			drawPanel.clear();
			drawData(self, world);
			YYY_TopLine topLine = YYY_Constants.getTopLine();
			mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(0,
			...
			mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(topLine.getLineDistance(),
																	0,
																	topLine.getLineDistance(),
																	game.getMapSize(),
																	Color.RED));
			...
			mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(game.getMapSize(),
																	0,
																	0,
																	game.getMapSize(),
																	Color.RED));
			YYY_BottomLine bottomLine = YYY_Constants.getBottomLine();
			mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(0,
																	bottomLine.getLineDistance(),
																	game.getMapSize(),
																	bottomLine.getLineDistance(),
																	Color.RED));
			mainFrame.getDrawPanel().addFigure(new YYY_Drawing_Line(bottomLine.getLineDistance(),
																	0,
																	bottomLine.getLineDistance(),
																	game.getMapSize(),
																	Color.RED));
		...
		if (move.getAction() == ActionType.FIREBALL) {
			double angle = YYY_Utils.normalizeAngle(self.getAngle() + move.getCastAngle());
			fireballShots.put(world.getTickIndex(),
							  new YYY_Pair<>(
									  new YYY_Point(self.getX() + Math.cos(angle) * move.getMaxCastDistance(),
		...
		if (move.getAction() == ActionType.MAGIC_MISSILE ||
			...
			double startAngle = YYY_Utils.normalizeAngle(self.getAngle() + move.getCastAngle());
			drawPanel.addFigure(new YYY_Drawing_Line(
	...
	private void drawUpdatedData(Wizard self) {
		for (YYY_BuildingPhantom buildingPhantom : enemyPositionCalc.getBuildingPhantoms()) {
			if (buildingPhantom.getFaction() == YYY_Constants.getCurrentFaction()) {
		...
		if (currentAction.getActionType().moveCalc) {
			...
			for (int i = 0; i != scan_matrix.length; ++i) {
				for (int j = 0; j != scan_matrix[0].length; ++j) {
					...
					drawPanel.addFigure(new YYY_Drawing_Circle(item.getX(),
															   item.getY(),
		...
		for (YYY_ScanMatrixItem scanMatrixItem : foundScanMatrixItems.values()) {
			drawCross(scanMatrixItem, 5., Color.MAGENTA);
		}

		YYY_Point filterPoint = new YYY_Point(self.getX() + Math.cos(direction) * YYY_Constants.MOVE_SCAN_FIGURE_CENTER,
											  self.getY() + Math.sin(direction) * YYY_Constants.MOVE_SCAN_FIGURE_CENTER);
		drawPanel.addFigure(new YYY_Drawing_Circle(filterPoint.getX(), filterPoint.getY(), YYY_Constants.MOVE_DISTANCE_FILTER, Color.red));
		drawPanel.addFigure(new YYY_Drawing_Circle(filterPoint.getX(), filterPoint.getY(), YYY_Constants.getFightDistanceFilter(), Color.red));
		...
		if (!wayPoints.isEmpty()) {
			Iterator<YYY_WayPoint> iterator = wayPoints.iterator();
			YYY_WayPoint prev, curr = null;
			if (iterator.hasNext()) {
				curr = iterator.next();
			}
		...
		Map.Entry<Integer, YYY_Pair<YYY_Point, Double>> fireballShotEntry = fireballShots.floorEntry(world.getTickIndex());
		if (fireballShotEntry != null) {
			int ticksToFly = YYY_Utils.getTicksToFly(fireballShotEntry.getValue().getSecond(), YYY_Utils.PROJECTIVE_SPEED[ProjectileType.FIREBALL.ordinal()]);
			if (fireballShotEntry.getKey() + ticksToFly >= world.getTickIndex()) {
				...
				drawPanel.addFigure(new YYY_Drawing_Circle(fireballShotEntry.getValue().getFirst().getX(),
		...
		textInfoPanel.putText(String.format("SpeedX: %s, SpeedY: %s, speed: %s",
											self.getSpeedX(),
											self.getSpeedY(),
											YYY_FastMath.hypot(self.getSpeedX(), self.getSpeedY())),
							  7);
		...
		if (currentAction.getActionType() == YYY_CurrentAction.ActionType.FIGHT) {
			YYY_Point selfPoint = scan_matrix[YYY_Constants.CURRENT_PT_X][YYY_Constants.CURRENT_PT_Y];
			if (maxAngle - minAngle > YYY_Constants.MOVE_ANGLE_PRECISE) {
				drawLine(selfPoint, YYY_Utils.normalizeAngle(self.getAngle() + angle + minAngle), 70., Color.black);
				drawLine(selfPoint, YYY_Utils.normalizeAngle(self.getAngle() + targetAngle), 70., Color.magenta);
				drawLine(selfPoint, YYY_Utils.normalizeAngle(self.getAngle() + angle + maxAngle), 70., Color.black);
			}
		}
		if (moveToPoint != null) {
			drawLine(new YYY_Point(self.getX(), self.getY()),
					 YYY_Utils.normalizeAngle(self.getAngleTo(moveToPoint.getX(), moveToPoint.getY()) + self.getAngle()),
					 50.,
					 Color.red);
		}
		...
		for (YYY_MinionPhantom minionPhantom : enemyPositionCalc.getDetectedMinions().values()) {
			...
			drawPanel.addFigure(new YYY_Drawing_Circle(minionPhantom.getPosition().getX(),
		...
		for (YYY_WizardPhantom wizardPhantom : enemyPositionCalc.getDetectedWizards().values()) {
			...
			if (checkDistance < 600.) {
				drawPanel.addFigure(new YYY_Drawing_Circle(wizardPhantom.getPosition().getX(),
	...
	private void drawTargets() {
		...
		YYY_TargetFinder.ShootDescription hasteTarget = null;
		if (!targetFinder.getHasteTargets().isEmpty() && YYY_Constants.getGame().getHasteManacost() <= self.getMana()) {
			hasteTarget = targetFinder.getHasteTargets().get(0);
		}

		YYY_TargetFinder.ShootDescription shieldTarget = null;
		if (!targetFinder.getShieldTargets().isEmpty() && YYY_Constants.getGame().getShieldManacost() <= self.getMana()) {
			shieldTarget = targetFinder.getShieldTargets().get(0);
		}

		if (missileShootDesc != null) {
			YYY_Point point = missileShootDesc.getShootPoint();
			if (point == null) {
				point = new YYY_Point(missileShootDesc.getTarget().getX(), missileShootDesc.getTarget().getY());
			}
			drawCross(point, 20., Color.MAGENTA);
		}
		if (staffHitDesc != null) {
			YYY_Point point = staffHitDesc.getShootPoint();
			if (point == null) {
				point = new YYY_Point(staffHitDesc.getTarget().getX(), staffHitDesc.getTarget().getY());
			}
			drawCross(point, 20., Color.BLACK);
		}
		if (iceShootDesc != null) {
			YYY_Point point = iceShootDesc.getShootPoint();
			if (point == null) {
				point = new YYY_Point(iceShootDesc.getTarget().getX(), iceShootDesc.getTarget().getY());
			}
			drawCross(point, 20., Color.BLUE);
		}
		if (fireShootDesc != null) {
			YYY_Point point = fireShootDesc.getShootPoint();
			if (point == null) {
				point = new YYY_Point(fireShootDesc.getTarget().getX(), fireShootDesc.getTarget().getY());
			}
		...
		if (hasteTarget != null) {
			...
			drawPanel.addFigure(new YYY_Drawing_Circle(target.getX(),
													   target.getY(),
													   target.getRadius() + 2.,
													   Color.MAGENTA));
		}
	...
	private void drawLine(YYY_Point pointA, YYY_Point pointB, Color color) {
		drawPanel.addFigure(new YYY_Drawing_Line(pointA.getX(),
												 pointA.getY(),
												 pointB.getX(),
												 pointB.getY(),
												 color));
	...
	private void drawLine(YYY_Point pointFrom, double angle, double distance, Color color) {
		drawPanel.addFigure(new YYY_Drawing_Line(pointFrom.getX(),
												 pointFrom.getY(),
	...
	private void drawCross(YYY_Point point, double size, Color color) {
		drawPanel.addFigure(new YYY_Drawing_Line(point.getX() - size,
												 point.getY() - size,
												 point.getX() + size,
												 point.getY() + size,
												 color));
	...
	protected void getBestMovePoint() {
		...
		for (int i = 0; i != scan_matrix.length; ++i) {
			for (int j = 0; j != scan_matrix[0].length; ++j) {
				...
				Map.Entry<Double, YYY_ScanMatrixItem> doubleScanMatrixItemEntry = foundScanMatrixItems.floorEntry(key);
	...
	private void drawData(Wizard self, World world) {
		// paint FOW
		double[] x = new double[]{0, 0, YYY_Constants.getGame().getMapSize(), YYY_Constants.getGame().getMapSize()};
		double[] y = new double[]{0, YYY_Constants.getGame().getMapSize(), YYY_Constants.getGame().getMapSize(), 0};
		drawPanel.addFigure(new YYY_Drawing_Polygon(x, y, true, Color.gray, -1000));
		for (Building unit : world.getBuildings()) {
			if (unit.getFaction() != YYY_Constants.getCurrentFaction()) {
				continue;
			}
			drawPanel.addFigure(new YYY_Drawing_Circle(unit.getX(), unit.getY(), unit.getVisionRange(), true, Color.white, -1000));
		}
		...
		YYY_Drawing_TextInfoPanel textPanel = mainFrame.getTextInfoPanel();
		textPanel.clear();
		...
		for (Tree unit : world.getTrees()) {
			...
			switch (YYY_Utils.whichLine(unit)) {
				case 0:
			...
			drawPanel.addFigure(new YYY_Drawing_Circle(unit.getX(),
													   unit.getY(),
													   unit.getRadius(),
													   true,
		...
		drawPanel.addFigure(new YYY_Drawing_Circle(self.getX(),
												   self.getY(),
												   self.getCastRange(),
												   false,
												   Color.blue));
	...
	public static YYY_Drawing_DrawingStrategy getInstance() {
		return instance;
	}
}
