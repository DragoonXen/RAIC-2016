 ...
 */
public class Drawing_DrawPanel extends JPanel {
    ...
    private List<Drawing_Figure> drawFigures;

	public Drawing_DrawPanel(double fullSizeX, double fullSizeY) {
		...
		drawFigures = new LinkedList<Drawing_Figure>();
	...
	private void recalcMultCoord() {
		...
		double multX = size.getWidth() / (Drawing_DrawPanel.this.drawWidth + 1);
		double multY = size.getHeight() / (Drawing_DrawPanel.this.drawHeight + 1);

		multCoord = Math.min(multX, multY);
		Drawing_DrawPanel.this.repaint();
	}

	private void zoomAction(int x, int y, boolean zoomIn) {
		double cX = x / multCoord;
		double cY = y / multCoord;
		cX += Drawing_DrawPanel.this.drawStartX;
		cY += Drawing_DrawPanel.this.drawStartY;
		if (zoomIn) {
    ...
    public void paint(Graphics g) {
        ...
        synchronized (this) {
            ...
            for (Drawing_Figure drawFigure : drawFigures) {
    ...
    public void addFigure(Drawing_Figure figure) {
        synchronized (this) {
