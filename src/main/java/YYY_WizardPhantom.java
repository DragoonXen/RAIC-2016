 ...
 */
public class YYY_WizardPhantom extends Wizard {
	...
	public YYY_WizardPhantom(Wizard wizard, int tick) {
		super(wizard.getId(),
			  wizard.getX(),
			  wizard.getY(),
			  wizard.getSpeedX(),
	...
	public YYY_WizardPhantom(Wizard wizard, int tick, boolean enemy) {
		super(wizard.getId() + (enemy ? (wizard.getId() > 5 ? -5 : 5) : 0),
			  enemy ? YYY_Constants.getGame().getMapSize() - wizard.getX() : wizard.getX(),
			  enemy ? YYY_Constants.getGame().getMapSize() - wizard.getY() : wizard.getY(),
			  wizard.getSpeedX(),
			  wizard.getSpeedY(),
			  wizard.getAngle(),
			  wizard.getFaction(),
	...
	public YYY_WizardPhantom clone() {
		YYY_WizardPhantom wp = new YYY_WizardPhantom(this, lastSeenTick);
		wp.position = new YYY_Point(position.getX(), position.getY());
		return wp;
	}
}
