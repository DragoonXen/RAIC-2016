 ...
 */
public class XXX_ScanMatrixItem extends XXX_Point {

	private int i, j;

	private XXX_ScanMatrixItem[] neighbours;

	private XXX_WayPoint wayPoint;
	...
	public XXX_ScanMatrixItem(int i, int j, double forwardDistanceDivision) {
		this.i = i;
		this.j = j;
	...
	public void setPoint(double x, double y) {
		...
		this.otherDanger = 0.;
		if (x + y < 627 || x + y + 627 > XXX_Constants.getDoubledMapSize()) {
			if (x > 2000) {
				x = XXX_Constants.getDoubledMapSize() - x - y;
			} else {
				x += y;
			}
			otherDanger = (627 - x) / 100 + 1.;
			otherDanger = (otherDanger * otherDanger - 1) * XXX_Constants.MAP_CORNER_DANGER_FACTOR;
		} else {
	...
	private void checkMapSideDanger(double coord) {
		if (coord > XXX_Constants.getGame().getMapSize() / 2) {
			coord = XXX_Constants.getGame().getMapSize() - coord - 1;
		}
		if (coord < XXX_Constants.MAP_SIDE_DANGER_DISTANCE) {
			otherDanger += (XXX_Constants.MAP_SIDE_DANGER_DISTANCE - coord) * XXX_Constants.MAP_SIDE_DANGER_FACTOR;
		}
	}
	...
	public void setNeighbours(XXX_ScanMatrixItem[] neighbours) {
	...
	public double getTotalScore(Wizard self) {
		...
		total -= dangers * dangers * XXX_Constants.DANGER_PENALTY;
	...
	public void setWayPoint(XXX_WayPoint wayPoint) {
		this.wayPoint = wayPoint;
	}

	public XXX_WayPoint getWayPoint() {
		return wayPoint;
	}
	...
	public XXX_ScanMatrixItem[] getNeighbours() {
		return neighbours;
	}
}
