 ...
 */
public class XXX_Drawing_TextInfoPanel extends JPanel {

	private List<XXX_Drawing_TextEntry> textEntryList;

	private Map<TextAttribute, Object> attributesMap;

	public XXX_Drawing_TextInfoPanel() {
		textEntryList = new LinkedList<XXX_Drawing_TextEntry>();
	...
	public void paint(Graphics g) {
		...
		int posY = 0;
		synchronized (this) {
			for (XXX_Drawing_TextEntry textEntry : textEntryList) {
	...
	public void putText(String text, int position) {
		synchronized (this) {
			while (textEntryList.size() <= position) {
				textEntryList.add(new XXX_Drawing_TextEntry(""));
			}
			Color color = Color.black;
			if (position < textEntryList.size()) {
				XXX_Drawing_TextEntry remove = textEntryList.remove(position);
				color = remove.getColor();
			}
			textEntryList.add(position, new XXX_Drawing_TextEntry(text, color));
		}
	}
}
