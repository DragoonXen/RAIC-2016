 ...
 */
public class Drawing_TextInfoPanel extends JPanel {

    private List<Drawing_TextEntry> textEntryList;

    private Map<TextAttribute, Object> attributesMap;

    public Drawing_TextInfoPanel() {
        textEntryList = new LinkedList<Drawing_TextEntry>();
    ...
    public void paint(Graphics g) {
        ...
        int posY = 0;
        synchronized (this) {
            for (Drawing_TextEntry textEntry : textEntryList) {
    ...
    public void putText(String text, int position) {
        synchronized (this) {
            while (textEntryList.size() <= position) {
                textEntryList.add(new Drawing_TextEntry(""));
            }
            Color color = Color.black;
            if (position < textEntryList.size()) {
                Drawing_TextEntry remove = textEntryList.remove(position);
                color = remove.getColor();
            }
            textEntryList.add(position, new Drawing_TextEntry(text, color));
        }
    }
}
